void createLocalA(Matrix &A,mesh m){
    float t = m.getParameter(CONST_T)/8;

    A.at(0).at(0) += -t; A.at(0).at(1) += t;
    A.at(1).at(0) += -t; A.at(1).at(1) += t;
}
...
void createLocalC(Matrix &C,mesh m){
    ...
    C.at(0).at(0) += -lambda; C.at(0).at(1) += lambda;
    C.at(1).at(0) += -lambda; C.at(1).at(1) += lambda;
...
void createLocalF(Matrix &F,mesh m){
    float alfa = (3*m.getParameter(ALFA))/2;

    F.at(0).at(0) += -alfa; F.at(0).at(1) += alfa;
    F.at(1).at(0) += -alfa; F.at(1).at(1) += alfa;
...
Vector createLocalb(int element,mesh &m){
    Vector b;
    float psi = m.getParameter(PSI), l = m.getParameter(ELEMENT_LENGTH), n = m.getParameter(CONST_N);
